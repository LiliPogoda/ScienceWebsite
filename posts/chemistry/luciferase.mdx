...
1. $\text{Luciferase} + \text{ATP} - \text{Mg}^+ + \text{luciferin} \to \text{Luciferase} + \text{luciferyladenylate} + \text{PP}_i-\text{Mg}^+$
2. $\text{luciferyladenylate} + \text{O}_{2} \to \text{oxyluciferin} + \text{AMP} + \text{\textit{light}} $

(TODO: Explain abbriviations)
...
that the dioxetanone undergoes a decarboxylation reaction for yield the light emitting exited oxyluciferin. If this then returns from its exited state to a relaxed ground state the emission of light happens.
