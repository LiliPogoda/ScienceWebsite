...
What makes a solution acidic or basic. As you might think it is not the compound itself that is acidic or basic. So pure Hydrogenchloride on its own is actually not acidic. What actual is acidic is the protons when the acid dissolves in a solution:
(TODO: ionisation HCL)
Because then the acid ionises. You might be wondering why in the equation it says H$_3$0$^+$. This is because in solution protons are in a hydrated form: h30+ cations also called hydroniumion.
...
How many of these ions are present in our solution depends on how willingly that acid or base dissolves. This can be explained with the equilibria of the reaction equations(TODO: read up here for in depth explanation od chemical equilibrias): Depending on wether we have more reactants or more products, we can determine how strong an acid or base is.
...
The concentration of H20 can be assumed as constant as the concentrations of H3o+ and OH- are very small. 
You can also that because $K_eq$ is very small, that means that the numerator has to be way smaller than the denominator. Therefore the concentration of water can be assumed as constant. Upon rearranging we get:
...
**$ 1.8 * 10^-16 * [55.5M] = [H^+][OH^-] = 10 * 10^-14 M^2 = K_w $** 
