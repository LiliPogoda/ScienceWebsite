...
export default function Chemistry({ source, title, date, author }) {
  return (
    <LayoutPostWrapper date={date} title={title} pageIdx={3}>
      <MDXRemote {...source} />
    </LayoutPostWrapper>
  );
}
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  const source = await getPost(path.join("programming", params.id as string));
...
export const getStaticPaths: GetStaticPaths = async () => {
  const paths = getAllPostIds("programming");
  return {
    paths,
    fallback: false,
  };
